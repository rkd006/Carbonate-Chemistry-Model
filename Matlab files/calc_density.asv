...
function density = calc_density(S, t, p)
...
p0 = pw + ((8.24493*10^(-1)) + (-4.0899*10^(-3))*t + (7.6438*10^(-5))*(t^2) + (-8.2467*10^(-7))*(t^3) + (5.3875*10^(-9))*(t^4))*S + ((-5.72466*10^(-3)) + (1.0227*10^(-4))*t...
    + (-1.6546*10^(-6))*(t^2))*(S^(3/2)) + (4.8314*10^(-4))*(S^2);
density = (p0)/(1-(p/Kp));
end